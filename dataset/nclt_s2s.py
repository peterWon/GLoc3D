...
odometry_dir = "/home/wz/Data/sdb1/wz/Data/uclt-odometry/dataset/"
...
# using the re-saved velodyne_sync_xyzi format, for faster reading.
def load_vel_bin(fname):
  hits = np.fromfile(fname, dtype=np.float32)
  return hits
  
  
def get_whole_training_set(onlyDB=False):
  structFile = join(struct_dir, 'uclt_s2s_pose_train_5.mat')
  return WholeDatasetFromStruct(
    structFile, input_transform = None, onlyDB = onlyDB)

def get_whole_val_set():
  structFile = join(struct_dir, 'uclt_s2s_pose_val_10.mat')
...
def get_val_query_set():
  structFile = join(struct_dir, 'uclt_s2s_pose_val_5.mat')
  return QueryDatasetFromStruct(
      structFile, input_transform = None)
...
class WholeDatasetFromStruct(data.Dataset):
  ...
  def __getitem__(self, index):
    lidar_data = load_vel_bin(self.scans[index]).reshape((-1, 4))
    num_points = lidar_data.shape[0]
    lidar_data = pad_or_trim_to_np(
      lidar_data, [61240, 4]).astype('float32')
    lidar_mask = np.ones(61240).astype('float32')
...
class QueryDatasetFromStruct(data.Dataset):
  def __init__(self, structFile, nNegSample=1000, nNeg=10, \
    ...
    # print(np.where(np.isnan(self.dbStruct.utmDb)))
    # print(np.where(np.isinf(self.dbStruct.utmDb)))
    knn = NearestNeighbors(n_jobs=1)
    knn.fit(self.dbStruct.utmDb)
  ...
  def __getitem__(self, index):
    ...
    query = load_vel_bin(join(queries_dir, \
      self.dbStruct.qLidar[index])).reshape((-1, 4))
    positive = load_vel_bin(join(root_dir, \
      self.dbStruct.dbLidar[posIndex])).reshape((-1, 4))
    
    np_q = query.shape[0]
    q_data = pad_or_trim_to_np(query, [61240, 4]).astype('float32')
    q_mask = np.ones(61240).astype('float32')
    q_mask[np_q:] *= 0.0

    np_p = positive.shape[0]
    p_data = pad_or_trim_to_np(positive, [61240, 4]).astype('float32')
    p_mask = np.ones(61240).astype('float32')
    p_mask[np_p:] *= 0.0
    ...
    for negIndex in negIndices:
      negative = load_vel_bin(join(root_dir, \
        self.dbStruct.dbLidar[negIndex])).reshape((-1, 4))
      np_n = negative.shape[0]
      n_data = pad_or_trim_to_np(negative, [61240, 4]).astype('float32')
      n_mask = np.ones(61240).astype('float32')
      n_mask[np_n:] *= 0.0
...
def generate_nclt_struct_files(dataset_type, skip_frames = 5, dist_threshold = 20):
  ...
  for seq in sequences:
    ...
    db_lidar_all.extend(
      [seq + '/' + 'velodyne_sync_xyzi' + '/' + fname for fname in vel_fnames_filter])
  ...
  savemat(os.path.join(root_dir, \
    'vlad_pose_dataset', 'uclt_s2s_pose_{}_{}.mat'.format(dataset_type, str(skip_frames))), mdict)



def view_dataset_split_trajectory():
  # val_dataset = get_whole_training_set()
