...
class PointPillarTest(nn.Module):
  def __init__(self, C, xbound, ybound, zbound, embedded_dim=16, direction_dim=37, cluster_mode = False, pose_mode = False, vlad_mode = False):
    super(PointPillarTest, self).__init__()
    self.xbound = xbound
    self.ybound = ybound
    self.zbound = zbound
    ...
    voxel_size_x = xbound[2]
    voxel_size_y = ybound[2]
    voxel_size_z = zbound[2]
    grid_size = np.asarray([
        (xbound[1]-xbound[0]) / voxel_size_x,
        (ybound[1]-ybound[0]) / voxel_size_y,
        (zbound[1]-zbound[0]) / voxel_size_z
    ]).astype('int32')
    ...
    self.grid_size = torch.from_numpy(grid_size)

  def forward(self, input):
    # print('before pointnet: ', points_feature.shape),torch.Size([4, 81920, 15])
    # points_feature, points_mask, voxel_indices = input[0], input[1], input[2]
    batch_size = input.shape[0]
    points_feature, voxel_indices, points_mask= input[:,:,:14], input[:,:,14].reshape(batch_size,-1).long(), input[:,:,15].reshape(batch_size,-1)
    points_feature = self.pn(points_feature, points_mask)
    # print('after pointnet: ', points_feature.shape),torch.Size([4, 64, 81920])

    # 取对应index的元素作平均
    voxel_feature = torch_scatter.scatter_mean(
      points_feature,
      torch.unsqueeze(voxel_indices, dim=1),
      dim=2,
    ...
    voxel_feature = voxel_feature.view(
      batch_size, -1, self.grid_size[0], self.grid_size[1])
...
class PointPillarVLAD(nn.Module):
  def __init__(self):
    super(PointPillarVLAD, self).__init__()
