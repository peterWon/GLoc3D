...
class NetVLAD(nn.Module):
    ...
    def __init__(self, num_clusters=64, dim=128, 
                 normalize_input=True, vladv2=False, gating=False):
        """
        Args:
            num_clusters : int
        ...
        self.hidden1_weights = nn.Parameter(torch.randn(
            num_clusters * dim, dim) * 1 / math.sqrt(dim))
        
        self.gating = gating
        if gating:
            self.context_gating = GatingContext(dim, add_batch_norm=True)
    ...
    def fix_weights(self):
      ...
      self.hidden1_weights.requires_grad = False
