...
#include "3d/port.h"
#include "grid_2d.h"
#include "2d/correlative_scan_matcher_2d.h"
#include "opencv2/opencv.hpp"
// #include "proto/scan_matching/fast_correlative_scan_matcher_options_2d.pb.h"
...
public:
  ...
  double linear_search_window() const {return linear_search_window_;}
  ...
  double linear_search_window_ = 3.;
  double angular_search_window_ = 3.;
...
class PrecomputationGrid2D {
 public:
  ...
  cv::Mat ToCvImage() const {
    cv::Mat img = cv::Mat(
        wide_limits_.num_y_cells, wide_limits_.num_x_cells, CV_8UC1);
    const int stride = wide_limits_.num_x_cells;
    for(size_t r = 0; r < wide_limits_.num_y_cells; ++r){
      for(size_t c = 0; c < wide_limits_.num_x_cells; ++c){
        img.at<uchar>(wide_limits_.num_y_cells - r - 1, c) = cells_[c + r * stride];
      }
    }
    return img;
  }
