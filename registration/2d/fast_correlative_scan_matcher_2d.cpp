...
bool FastCorrelativeScanMatcher2D::MatchFullSubmap(
  ...
  const SearchParameters search_parameters(
      25 * limits_.resolution(),  // Linear search window, 1e6 cells/direction.
  ...
  // LOG(INFO)<<center;
  // LOG(INFO)<<search_parameters.num_angular_perturbations;
  // LOG(INFO)<<search_parameters.resolution;
  // LOG(INFO)<<search_parameters.angular_perturbation_step_size;
...
bool FastCorrelativeScanMatcher2D::MatchWithSearchParameters(
  ...
  //
  /* const auto& lowest_best_candidate = lowest_resolution_candidates.front();
  if (lowest_best_candidate.score > 0.5) {
    *score = lowest_best_candidate.score;
    *pose_estimate = transform::Rigid2d(
        {initial_pose_estimate.translation().x() + lowest_best_candidate.x,
         initial_pose_estimate.translation().y() + lowest_best_candidate.y},
        initial_rotation * Eigen::Rotation2Dd(lowest_best_candidate.orientation));
    return true;
  } */
